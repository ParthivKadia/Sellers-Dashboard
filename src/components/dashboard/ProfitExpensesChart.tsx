...
const monthlyData = [
  ...
  { month: "Dec", revenue: 310000, cogs: 155000, opex: 46500 },
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-card border border-border rounded-lg px-3 py-2.5 text-xs shadow-lg space-y-1">
      ...
      {payload.map((p: any) => (
        <p key={p.name} style={{ color: p.color }}>{p.name}: ₹{p.value}K</p>
      ))}
    </div>
  );
};
...
const ProfitExpensesChart = () => (
  <div className="bg-card rounded-xl p-4 sm:p-6 col-span-full lg:col-span-7">
    ...
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-5">
      ...
      <div className="flex gap-3 text-[11px] text-muted-foreground flex-wrap">
        ...
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-destructive/70 inline-block"/>Expenses</span>
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-emerald-500 inline-block"/>Net Profit</span>
      </div>
    </div>

    {/* Chart */}
    <div className="h-[180px] sm:h-[200px] mb-5">
    ...
    {/* ── P&L Table — scrollable on small screens ── */}
    <div className="border border-border rounded-lg overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[480px] text-xs">
          <thead>
            <tr className="bg-secondary/60">
              <th className="text-left py-2.5 px-3 sm:px-4 font-semibold text-muted-foreground whitespace-nowrap">P&L Item</th>
              <th className="text-right py-2.5 px-3 sm:px-4 font-semibold text-muted-foreground whitespace-nowrap">8-Month Total</th>
              <th className="text-right py-2.5 px-3 sm:px-4 font-semibold text-muted-foreground whitespace-nowrap">Mar (Current)</th>
              <th className="text-right py-2.5 px-3 sm:px-4 font-semibold text-muted-foreground whitespace-nowrap">MoM</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {[
              { label: "Revenue",            total: totalRevenue, cur: cur.revenue,                       mom: momRevChange, color: "text-foreground",       bold: true  },
              { label: "Cost of Goods",      total: totalCOGS,   cur: cur.cogs,                          mom: null,         color: "text-muted-foreground",  bold: false },
              ...
              <tr key={row.label} className={`${row.bold ? "font-semibold" : ""} hover:bg-secondary/30 transition-colors`}>
                <td className="py-2.5 px-3 sm:px-4 text-foreground whitespace-nowrap">{row.label}</td>
                <td className="py-2.5 px-3 sm:px-4 text-right text-foreground whitespace-nowrap">{fmt(row.total)}</td>
          ...
          <tfoot>
            <tr className="bg-secondary/60 border-t border-border">
              <td colSpan={2} className="py-2.5 px-3 sm:px-4 text-[11px] text-muted-foreground whitespace-nowrap">
              ...
              <td colSpan={2} className="py-2.5 px-3 sm:px-4 text-right">
                <button className="bg-primary text-primary-foreground text-[11px] font-semibold px-3 py-1.5 rounded-md hover:opacity-90 transition whitespace-nowrap">
                  Export Report
                </button>
              </td>
            </tr>
